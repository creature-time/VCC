...
namespace CreatureTime
{
    ...
    public class CtStateMachine : CtAbstractSignal
    {
        ...
        private void _NextNode(int identifier, CtStateBase nextState)
        {
            ...
            {
#if DEBUG_LOGS
                LogDebug($"Leaving state (identifier={identifier}, prevState={prevState}-{prevState.GetUdonTypeName()}).");
#endif
            ...
            {
#if DEBUG_LOGS
                LogDebug($"Entering state (identifier={identifier}, nextState={nextState}-{nextState.GetUdonTypeName()}))");
#endif
